...
{
    assert(pixels);
    __NS::handleBitmap((__NS::Color*)pixels, width, height, [](__NS::Color& color) { color.red = color.green = color.blue = (uint8_t)(0.30f * color.red + 0.59f * color.green + 0.11f * color.blue); });
}
