...
///////////////////////////////////////////////////////////////////////////////
// Implementation of the atomic_t class
//

template <typename _Ty>
__INLINE__ atomic_t<_Ty>::atomic_t(_Ty value/* = 0*/)
    : mValue(value)
{
}

template <typename _Ty>
__INLINE__ atomic_t<_Ty>::atomic_t(const atomic_t<_Ty>& that)
    : mValue(that.mValue)
{
}

template <typename _Ty>
__INLINE__ atomic_t<_Ty>::operator _Ty() const
{
    return mValue;
}

template <typename _Ty>
__INLINE__ atomic_t<_Ty>& atomic_t<_Ty>::operator=(_Ty value)
{
    ::__atomic_swap(value, &mValue);
    return *this;
}

template <typename _Ty>
__INLINE__ atomic_t<_Ty>& atomic_t<_Ty>::operator=(const atomic_t<_Ty>& that)
{
    ::__atomic_swap(that.mValue, &mValue);
    return *this;
}

template <typename _Ty>
__INLINE__ _Ty atomic_t<_Ty>::operator++()
{
    ::__atomic_inc(&mValue);
    return mValue;
}
...
template <typename _Ty>
__INLINE__ _Ty atomic_t<_Ty>::operator++(int)
{
    return ::__atomic_inc(&mValue);
}
...
__INLINE__ _Ty atomic_t<_Ty>::operator--(int)
{
    return ::__atomic_dec(&mValue);
}

template <typename _Ty>
__INLINE__ _Ty atomic_t<_Ty>::operator+=(_Ty value)
{
    ::__atomic_swap(mValue + value, &mValue);
    return mValue;
}
...
template <typename _Ty>
__INLINE__ _Ty atomic_t<_Ty>::getAndAdd(_Ty value)
{
    return ::__atomic_swap(mValue + value, &mValue);
}

template <typename _Ty>
__INLINE__ _Ty atomic_t<_Ty>::getAndSet(_Ty value)
{
    return ::__atomic_swap(value, &mValue);
}

template <typename _Ty>
__INLINE__ int atomic_t<_Ty>::compareAndSet(_Ty oldValue, _Ty newValue)
{
    return ::__atomic_cmpxchg(oldValue, newValue, &mValue);
}

#ifndef NDEBUG
template <typename _Ty>
__INLINE__ void atomic_t<_Ty>::dump() const
{
    LOGI("atomic_t [ value = %d ]\n", mValue);
}
#endif  // NDEBUG
